...
*Примечание: в тесте `cat` наблюдается маленький прирост производительности (1.2%). Это связано с накладными расходами на механизм суперскалярности в условиях частых ветвлений. Перед любой инструкцией, меняющей поток управления (`JUMP`, `BEQZ`, `CALL` и т.д.), процессор обязан принудительно сбросить (flush) теневой регистр `AC_SHADOW` в память, что занимает 1 дополнительный такт. Код `cat` состоит из очень короткого цикла с частыми проверками условий, где полезных параллельных операций почти не происходит, а накладные расходы на сброс теневого регистра перед прыжками почти перевешивают выгоду.*