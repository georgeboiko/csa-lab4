...
Прерывание дополнительно добавляет **12 тактов** на сохранение состояния
...
### Оптимизации в кодогенераторе

Перед тем как порождать «канонический» развёрнутый код для каждого
Forth-слова, [`CodeGenerator.visit_sequence()`](utils/codegen.py)
пытается матчить узлы AST с набором паттернов. Если паттерн сошёлся — несколько узлов
сворачиваются в существенно более короткую цепочку инструкций.
Реализованы такие паттерны:

| Паттерн на Forth        | Что генерируется наивно | Что генерирует оптимизатор | Экономия |
|-------------------------|-------------------------|----------------------------|----------|
| `var @ N swap !`        | push var-адр., `@`, push N, swap, `!` (около 14 инстр.) | `LOAD_IMM N; STORE_IND var` | 12+ |
| `var1 @ N + var2 !`     | полная развёртка `@`/`+`/`!` через стек | `LOAD var1; ADD_IMM N; STORE var2` | ~10 |
| `var1 @ var2 !`         | `@` + `!` через стек | `LOAD var1; STORE var2` | ~10 |
| `var @ emit`            | `@` + `emit` через стек | `LOAD var; STORE OUTPUT_ADDR` | ~5 |
| `var @ !`               | три обращения к стеку данных | `load_sp; STORE_IND var; inc_sp` | ~6 |
| `var @ @`               | косвенная загрузка через стек | `LOAD var; LOAD_ACC; push_acc` | ~4 |
| `N var !`               | push N, push var-адрес, `!` | `LOAD_IMM N; STORE var` | ~6 |
| `var @`                 | push var-адрес, `@` | `LOAD var; push_acc` | ~3 |
| `1 +` / `1 -`           | push 1, `+` / `-` | `load_sp; INC/DEC; store_sp` | ~5 |
| `N + / N -` (N ≥ 2)     | push N, `+` / `-` | `load_sp; ADD_IMM/SUB_IMM N; store_sp` | ~5 |
| `N =` (immediate cmp)   | push N, `=` (с TEMP-обменом) | `load_sp; SUB_IMM N; BEQZ …` | ~4 |
| `dup @`                 | `dup` + `@` через стек | `load_sp; LOAD_ACC; push_acc` | ~4 |

Дополнительно генератор:

- **Дедуплицирует имена функций без рекурсии**: имя функции
  временно скрывается из таблицы `functions` на время компиляции её
  тела, что отрезает случайные рекурсивные `CALL` и позволяет
  inline-обработку без специальных проверок (видно в
  [`CodeGenerator.visit()`](utils/codegen.py) для `FuncDefNode`).
- **Размещает литералы и строки последовательно с указателем
  `data_ptr`**, начиная с `TEMP2_SAVE_ADDR + 1` — никакой
  фрагментации статической памяти не возникает.
- **Откладывает разрешение меток до отдельного линкер-прохода**
  ([`CodeGenerator.resolve_labels()`](utils/codegen.py)), что
  позволяет паттерн-матчеру свободно генерировать вперёд-ссылки на
  ещё не определённые `else` / `endif` / `begin` без обратной
  правки уже выпущенных инструкций.
...
* **MMIO**: `mem[INPUT_ADDR=4]` — порт ввода, `mem[OUTPUT_ADDR=5]` — порт вывода. Слово `key` компилируется в `LOAD 4`, `emit` — в `STORE 5`.
* **IVT**: `mem[IVT_INPUT_ADDR=0]` хранит байтовый адрес ISR (заполняется кодогенератором при наличии `:isr`). Если `IVT[0]==0`, прерывания не включаются и расписание ввода игнорируется.

### Доставка символов (host-side, [`machine.py`](machine.py:50))

Цикл симуляции на каждом такте проверяет расписание и доставляет символы, чьё время пришло.

Следствие: если в расписании два символа на близких / соседних тактах, а ISR ещё не успел прочитать `mem[INPUT_ADDR]`, **новое значение перетирает старое** — символ теряется. Это естественное поведение MMIO-порта без буфера/очереди. Программа должна укладываться в межсимвольный интервал, иначе будет drop.

### Поведение CU при IRQ

Прерывания проверяются в **начале каждого такта** в [`ControlUnit.process_next_tick()`](control_unit.py:150). Условие захода в ISR:

```
IE == 1  AND  IRQ == 1  AND  IVT[0] != 0
```

Если условие выполнено **в середине** многотактовой инструкции (`step > 0`), эта инструкция **отбрасывается**: её внутренние шаги не сохраняются, частичные эффекты в DataPath игнорируются (фактические записи в память происходят только в финальных шагах, поэтому отбрасывание безопасно). CU запоминает «точку возврата» в `_restart_pc`:

* `step == 0` → `_restart_pc = PC` (инструкция ещё не начиналась, вернёмся к ней же).
* `step > 0` → `_restart_pc = PC − 4` (PC уже инкрементировался при fetch; нужно перевыполнить инструкцию с нуля).

После этого `step` сбрасывается в 0 и запускается **12-тактовый** [`_isr_entry_tick()`](control_unit.py:211):

| Шаг | Действие |
|-----|----------|
| 0 | Сброс `AC_SHADOW` (если был не пуст) |
| 1–2 | `SP -= 4`, запись `_restart_pc` в `mem[SP]` (push на стек возвратов) |
| 3–4 | Сохранение `ACC → mem[ACC_SAVE_ADDR=7]` |
| 5–6 | Сохранение `ACC_ADDR → mem[ACC_ADDR_SAVE_ADDR=8]` |
| 7–8 | Сохранение `NZVC → mem[NZVC_SAVE_ADDR=9]` |
| 9–10 | Чтение `ISR_ADDR ← mem[IVT_INPUT_ADDR=0]` |
| 11 | `PC ← ISR_ADDR`, `IE ← 0`, `IRQ ← 0` |

Внутри ISR `IE=0`, поэтому вложенные прерывания невозможны.

### Выход из прерывания (IRET)

Опкод `IRET` выполняется за **13 тактов** ([`_step_iret()`](control_unit.py:486)) и зеркально откатывает вход:

1. Flush `AC_SHADOW`.
2. Восстановление `NZVC ← mem[9]`, `ACC_ADDR ← mem[8]`, `ACC ← mem[7]`.
3. Pop с return-stack: чтение `PC ← mem[SP]`, затем `SP += 4`.
4. `IE ← 1`.

После `IRET` управление возвращается на `_restart_pc`, и прерванная инструкция **выполняется с шага 0 заново** — поэтому отбрасывание её частичного состояния корректно: повтор даст тот же результат.
...
7. **DR и Data Memory**
   - DR связан с памятью данных через мультиплексор - читается значение из памяти, либо, если в теневом аккумуляторе лежит валидное значение, то оно (для поддержания консистентности данных):
     * захватывает данные по сигналу `latch_dr`
     * выход идет в верхний MUX перед ALU
   - От DR линия идет также в CU как `dr_to_cu`
...
| Кейс              | Файл | Описание                                                            |
|-------------------|------|---------------------------------------------------------------------|
| `hello`           | [`golden/hello.yml`](golden/hello.yml) | «Hello, World!» через статическую строку и `emit` |
| `cat`             | [`golden/cat.yml`](golden/cat.yml)     | Чтение из ввода до символа `\n`, перевывод (echo) |
| `hello_user_name` | [`golden/hello_user_name.yml`](golden/hello_user_name.yml) | Печать prompt, чтение имени, печать приветствия |
| `prob1`           | [`golden/prob1.yml`](golden/prob1.yml) | Project Euler #4: наибольшее произведение-палиндром |
| `sort`            | [`golden/sort.yml`](golden/sort.yml)   | Пузырьковая сортировка массива из ввода |
| `double_precision`| [`golden/double_precision.yml`](golden/double_precision.yml) | Сложение/вычитание двух 64-битных чисел через флаги C/V |
| `execution_token` | [`golden/execution_token.yml`](golden/execution_token.yml) | Демонстрация работы с execution token |
...
### Почему ускорение именно такое

Теневой аккумулятор экономит такты только на парах
`STORE addr` → `LOAD addr` (или `STORE addr` → `ADD/SUB/MUL/… addr`),
причём адреса в этих парах должны совпадать или хотя бы попадать в
`AC_SHADOW` до того, как туда попало значение для другого адреса.
Все остальные ускорения отсутствуют по построению:

* `LOAD_IMM`, `ADD_IMM`, `SUB_IMM`, `INC`, `DEC`, `SHIFTL/R`, `NOT`,
  `CLC/CLV`, ветвления и `JUMP` уже занимают 2 такта в baseline и
  не имеют куда укорачиваться.
* `CALL` / `RET` / `IRET` в худшем случае ещё **дороже** в
  суперскалярном режиме (+2 такта на `flush AC_SHADOW`), если
  перед вызовом в shadow осталось грязное значение.
* Все обращения к MMIO (`INPUT_ADDR=4`, `OUTPUT_ADDR=5`) принудительно
  идут мимо shadow — иначе можно было бы потерять байт ввода-вывода,
  поэтому `emit` и `key` суперскаляр не ускоряет вообще.

Отсюда понятна картина:

| Кейс              | Ускорение | Почему столько                                                       |
|-------------------|-----------|----------------------------------------------------------------------|
| `execution_token` | **2.0 %** | Очень короткая программа, большая её часть — `CALL_ACC` / `RET`; на управляющих инструкциях экономии нет. |
| `prob1`           | **3.9 %** | Главный цикл — вложенные счётчики и проверка палиндрома. Много `LOAD/STORE` к разным переменным подряд, shadow часто приходится сбрасывать parallel-flush'ом → выгода съедается. |
| `double_precision`| **5.2 %** | Длинная цепочка `ADD/SUB addr` над одной и той же hi/lo-парой, но между ними `CLC/CLV/SHIFTL/SHIFTR`, которые не пользуются shadow. |
| `sort`            | **5.4 %** | Пузырьковая сортировка много раз пишет и сразу читает `mem[i]`/`mem[i+1]` — это идеальный кейс для shadow-forward, но цикл сравнения тоже тратит много тактов на ветвления. |
| `cat`             | **5.8 %** | Ввод и вывод идут через MMIO мимо shadow; ускоряется только работа с буфером ring-buffer'а. |
| `hello_user_name` | **8.8 %** | Длинная статическая строка приветствия печатается циклом «`@`+`emit`», между ними классическая пара `STORE`→`LOAD` по тому же указателю-курсору. |
| `hello`           | **9.8 %** | Самый «чистый» сценарий: проход по C-строке курсором, между итерациями `STORE p` → `LOAD p` — shadow срабатывает почти всегда. |

Иными словами, доля ускоряемых инструкций в типичной программе
для нашей ISA редко превышает 10–15 %, и именно поэтому
агрегированный выигрыш по тактам стабильно держится в коридоре
**2 — 10 %** в зависимости от характера нагрузки.